...
#listfile = "rock-metal-country-artists.list"
listfile = "rock-metal-country-artists-current.list"
artist_websites = set()
lyrics_map = {}
...
try:
	if sys.argv[1] == "2": listfile = "rock-metal-country-artists-current2.list"
...
def get_mldb_song_urls(artist_url, page, artist):
	...
	for w in wp:
		try:
			...
			print(artist)
			cleaned_artist = clean_name(artist.lower().replace("the ","").strip())
...
if __name__ == "__main__":
	...
	print("threshhold is "+str(threshhold)+" and wait is "+str(seconds_to_wait))
